# Forward Simulation 1850-2500 (GISS SSP2-4.5, r3i1p1f2)
# 50-year segmented solves
...
my_function_path = '/home/yanmeiti/issm/Greenland/Functions'
...
# =============================================================
# Configuration — GISS ensemble member
# =============================================================
MEMBER   = 'r5'          
CMIP_MEMBER = 'r5i1p1f2' 
MODELS   = '/home/yanmeiti/issm/Greenland/Model'
GISS_BASE = f'/home/yanmeiti/issm/Greenland/Data/{MEMBER}'
SMR_BASE  = f'/home/yanmeiti/issm/Greenland/Data/{MEMBER}'
MAR_FILE  = '/home/yanmeiti/issm/Greenland/Data/MAR_1981_2010_monthly_climatology_for_ISSM.nc'
DELTA18O  = '/home/yanmeiti/issm/Greenland/Data/delta18o.data'
RELAX_NC  = f'{MODELS}/Step2.2_relaxation_calving_200yr.nc'   # ice model state (same for all members)

H = f'{GISS_BASE}/tas_Amon_GISS-E2-1-G_historical_{CMIP_MEMBER}_gn_'
P = f'{GISS_BASE}/pr_Amon_GISS-E2-1-G_historical_{CMIP_MEMBER}_gn_'
F = f'{GISS_BASE}/tas_Amon_GISS-E2-1-G_ssp245_{CMIP_MEMBER}_gn_'
Q = f'{GISS_BASE}/pr_Amon_GISS-E2-1-G_ssp245_{CMIP_MEMBER}_gn_'
...
ALL_SMR_FILES = [
    (1850, 2014, _smr_path(SMR_BASE, CMIP_MEMBER, 1850, 2014), 1850.0),
    (2015, 2100, _smr_path(SMR_BASE, CMIP_MEMBER, 2015, 2100), 2015.0),
    (2101, 2300, _smr_path(SMR_BASE, CMIP_MEMBER, 2101, 2300), 2101.0),
    (2301, 2500, _smr_path(SMR_BASE, CMIP_MEMBER, 2301, 2500), 2301.0),
]
...
# Read the existing historical chunks that cover the reference interval.
# =============================================================
print('Computing 1981-2010 reference climatology...')

ref_chunks = ALL_GISS_FILES[2:4]  # 1951-2000 and 2001-2014
tas_parts = []
pr_parts = []
date_parts = []
for _, _, tas_file, pr_file, _ in ref_chunks:
    with Dataset(tas_file) as nc:
        ...
        tas_parts.append(nc.variables['tas'][:])
        date_parts.extend(dates)
    with Dataset(pr_file) as nc:
        pr_parts.append(nc.variables['pr'][:])
tas_all = np.concatenate(tas_parts, axis=0)
pr_all = np.concatenate(pr_parts, axis=0)
...
years_all = np.array([date.year for date in date_parts], dtype=int)
...
def build_smr(smr_files, smr_year_range):
    ...
    for smr_file, epoch_yr in smr_files:
        ...
        dt = float(t_raw[1] - t_raw[0]) if len(t_raw) > 1 else 30.0
        if dt > 10:  # days (monthly spacing ~30 d): r1/r2 convention
            t = epoch_yr + t_raw / 365.25
        else:        # month indices (spacing = 1): r3 convention, global epoch=1850
            t = 1850.0 + t_raw / 12.0
        yi_int = t.astype(int)
